...
public class FieldRequestValidator implements Validator {
	...
	public boolean supports(Class<?> clazz) {
		return clazz.isAssignableFrom(FieldRequest.class);
	}

	@Override
	public void validate(Object target, Errors errors) {
		FieldRequest req = (FieldRequest) target;
		...
		if (genType != null && type != null) {
			...
			else if(genType.equalsIgnoreCase("CustomBehaviour"))
				ValidationUtils.rejectIfEmptyOrWhitespace(errors, "customBehaviour.simulations", "field.required", "The simulations number is required");
	...
	private void fixedValidations(Errors errors, FieldRequest req) {
	...
	private void randomValidations(Errors errors, FieldRequest req) {
		if (Arrays.asList("Integer", "Long", "Float").contains(req.getType())) {
	...
	private void sequentialValidations(Errors errors, FieldRequest req) {
