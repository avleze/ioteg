...
{
    private static final Log log = LogFactory.getLog(RuntimeConfigMain.class);
    ...
    private void configureVariables() {
        ...
        EPStatement stmt = provider.getEPAdministrator().createEPL("select propertyOne, propertyTwo, myintvar from MyEvent");
    ...
    private void configureSingleRowFunction() {
        ...
        // keep the last few events from the variant stream
        EPStatement stmt = provider.getEPAdministrator().createEPL("select mymedian(doubles) as med from MyMedianSampleEvent");
    ...
    private void configureImport() {
        ...
        // keep the last few events from the variant stream
        EPStatement stmt = provider.getEPAdministrator().createEPL("select RuntimeConfigMain.check2BitSet(byteValue) as check2BitSet from MyByteEvent");
